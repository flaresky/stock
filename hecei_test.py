...
from_date = date(2001, 11, 1)
end_date = date(2016, 6, 1)
dingtou_money = 3000
pe_config = (10, 15) #pe配置，9以下定投，10以上全部卖掉，9-10之间买回卖掉的
g_raise_step = 0.1
g_sell_step = 0.05
...
def get_pe_by_date(d):
    while True:
        ...
        sql = "select pe from hscei_pe where `date`='%s'"%(str(d));
...
def get_point_by_date(d):
    while True:
        sql = "select close from hscei_history where `date`='%s'"%(str(d));
        db_res = mysqllib.fetch_one(sql)
        if db_res is not None:
            return db_res['close']
        d = d + timedelta(days=1)
